...
on the page as plain scripts, and the preview reloads on its own — there is nothing to install,
compile or bundle. **Build the user's app by editing `static/app.js` (and the files beside it),
and `app.py` when the app needs a route of its own.**
