...
# no store anywhere near it. `FlightExecutor` below is the real one; the server's `main` wires it.
...
def unavailable_executor(query: Query, params: dict) -> dict:
    ...
    The server's `main` wires `FlightExecutor`, so a published app does not land here. What does is a
    server built by a test, and a caller who deliberately passed nothing — both of which are better
    answered by saying there is no data than by a fake that would answer a viewer's real question
    with invented rows.
    """
    raise QueryProblem(HTTPStatus.SERVICE_UNAVAILABLE,
...
def answer(queries: dict, executor, name: str, body: object) -> tuple[int, dict]:
    """One named-query request, answered: the HTTP status and the JSON payload to send.

    Transport-neutral on purpose. `QueryRoute` below calls it from a stdlib handler, and a stack whose
    server is a FastAPI app calls it from a route — so two servers cannot disagree about what a
    ...
    try:
        ...
        params = query.bind(body.get("params") if isinstance(body, dict) else None)
        result = (executor or unavailable_executor)(query, params)
    except QueryProblem as e:
        return int(e.status), {"error": e.message}
    except Exception as e:   # an executor that failed in a way it did not describe  # noqa: BLE001
        print(f"[sage] query {name} failed: {type(e).__name__}: {e}", flush=True)
        return int(HTTPStatus.BAD_GATEWAY), {"error": "This app could not read its data source."}
    return int(HTTPStatus.OK), query_response(query, result)
...
class QueryRoute:
    """The named-query route, as a mixin for a `BaseHTTPRequestHandler`.

    Reads the catalog and the executor off the server it is on (`sage_queries`, `sage_executor`), so
    the handler class that mixes this in decides nothing about queries: `serve.py`'s static handler
    and the query-only handler below answer the route identically because it is the same code. The
    `[sage]` log lines live here too — Domino surfaces the App's stdout, so that is where problems go,
    whichever handler saw them.
    """

    def do_POST(self):
        """The named-query route, and the only method a server here answers besides GET and HEAD."""
        ...
        status, payload = answer(getattr(self.server, "sage_queries", {}),
                                 getattr(self.server, "sage_executor", unavailable_executor), name, body)
        return self._send_json(status, payload)

    def query_name(self) -> str:
        """The name in the path, or "" when this is not the query route."""
        return urlsplit(self.path).path[len(_API_PREFIX):] if self._is_query_path() else ""
...
class _QueryOnlyHandler(QueryRoute, BaseHTTPRequestHandler):
    """The route alone, for Sage's preview (#24). The page is the dev server's to serve there, so
    everything that is not a query is a 404 rather than a file."""
    ...
    def do_GET(self):
        # A query path is POST-only, and saying so beats a 404 that reads as a missing query.
        if self._is_query_path():
            self._send_json(HTTPStatus.METHOD_NOT_ALLOWED, {"error": "This endpoint takes POST."})
        else:
            self._send_json(HTTPStatus.NOT_FOUND, {"error": "No such endpoint."})


def build_server(*, host: str = "127.0.0.1", port: int = 0, project_root: Path | str = ".",
                 executor=None) -> ThreadingHTTPServer:
    """A bound (not yet serving) server answering the query route and nothing else.

    This is what Sage's preview runs on loopback (#24). `serve.py` builds its own around the static
    handler, with the same two attributes — the route reads `sage_queries` and `sage_executor` off
    whichever server it is on. `project_root` is where the app's `.sage/` manifests live. `executor`
    is the seam: a callable taking (query, params) and returning {columns, rows}, injected rather
    than imported so a test can prove the boundary without a store.
    """
    srv = ThreadingHTTPServer((host, port), _QueryOnlyHandler)
    srv.sage_queries = load_queries(Path(project_root))
    srv.sage_executor = executor or unavailable_executor
    return srv


def log_query_catalog(queries: dict) -> None:
