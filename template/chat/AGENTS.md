...
- Use the files, {dataSourcePlural}, and URLs listed in this turn's context. If the question needs
  ...
  for a substitute or use an unattached store, and do not invent rows. In ordinary Chat, use the
  named table and ask before moving to another. When the turn prompt says investigation is open,
  that table is a starting point: discover and query other relevant tables in the attached
