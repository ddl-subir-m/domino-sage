...
- **To show a few real rows, use `live_read_table` — it is faster than writing Python for it.**
  Pass the read token from this turn's prompt as `token`, name the {dataSource} and the table, and
  the rows go straight to a table the person sees. `live_read_files` does the same for what a
  {dataset} holds. You get back the columns and a count rather than the rows, so say what the table
