...
def _root(orch) -> Path:
    return orch.project(start_preview=False).record.path


def _read(rows: list[dict], root: Path, *, source: str, turn_id: str, sql: str) -> dict:
    ...
    The card is written under the PROJECT ROOT, where a real turn writes it, because the prompt
    filters these paths to the files this clone actually has. Writing it to a scratch directory
    instead would leave every path dead and the block permanently pathless — green, and testing
    nothing.
    """
    turn, recorded = turn_for(root, keep_rows=True,
                              answer=FakeAnswer(["N"], [[41234]]),
                              examples_dir=root / "examples" / TID,
    ...
    assert (root / event["artifact"]).exists(), "the producer wrote the card somewhere else"
...
def test_each_source_gets_one_line_naming_its_turns_and_its_latest_result(tmp_path: Path):
    ...
    root = _root(orch)
    rows: list[dict] = []
    _read(rows, root, source="SFDC__ACCOUNT", turn_id="t1", sql="SELECT 1")
    _read(rows, root, source="SFDC__ACCOUNT", turn_id="t2", sql="SELECT 2")
    last = _read(rows, root, source="GONG__ACCOUNT_TRACKER_DAILY", turn_id="t2",
                 sql="SELECT 3")
...
def test_one_turns_many_model_calls_are_not_counted_as_turns(tmp_path: Path):
    ...
    root = _root(orch)
    rows: list[dict] = []
    row = _read(rows, root, source="SFDC__ACCOUNT", turn_id="t1", sql="SELECT 1")
...
def test_one_turns_several_reads_of_one_source_are_not_counted_as_turns(tmp_path: Path):
    ...
    rows: list[dict] = []
    _read(rows, root, source="SFDC__ACCOUNT", turn_id="t1", sql="SELECT 1")
    _read(rows, root, source="SFDC__ACCOUNT", turn_id="t1", sql="SELECT 2")
...
def test_a_re_persisted_old_read_does_not_become_the_newest(tmp_path: Path):
    ...
    root = _root(orch)
    rows: list[dict] = []
    old = _read(rows, root, source="SFDC__ACCOUNT", turn_id="t1", sql="SELECT 1")
    _read(rows, root, source="GONG__CALLS", turn_id="t2", sql="SELECT 2")
...
def test_the_block_is_capped_and_newest_first(tmp_path: Path):
    ...
    rows: list[dict] = []
    for i in range(25):
        _read(rows, root, source=f"TABLE_{i:02d}", turn_id=f"t{i}", sql=f"SELECT {i}")
...
def test_a_result_whose_file_is_gone_is_not_handed_over_as_a_path(tmp_path: Path):
    """The history row outlives the file, so the event's `artifact` is a claim about disk.

    The Artifact list below this block is filtered by `_artifacts_present` for a stated reason: a
    dead path costs a tool call and an answer written around a file that is not there. This block
    hands over paths too, so it goes through the same filter.

    The line still renders. That the source was read is true whether or not the card survived, and
    it is the dead PATH that costs a turn, not the missing one.
    """
    orch, _ = _orch(tmp_path)
    root = _root(orch)
    rows: list[dict] = []
    row = _read(rows, root, source="SFDC__ACCOUNT", turn_id="t1", sql="SELECT 1")
    artifact = row["dataUsed"][0]["artifact"]
    assert artifact in _prompt(orch, history=rows), "the live path is handed over"
    ...
    assert "- SFDC__ACCOUNT — read on 1 turn" in prompt
