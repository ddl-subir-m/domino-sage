...
def test_the_rendered_history_names_the_packs_assistant_but_quotes_the_agent_verbatim(
    tmp_path: Path, monkeypatch
):
    """`.sage/history.md` is regenerated into a repo the user owns and publishes, so the chrome
    around the record — the generated-by comment and the byline — re-brands (#110). What the agent
    actually said does not: a transcript is a record, and regeneration reproduces speech."""
    import json
    ...
    assert "<!-- Generated by Ada from .sage/history.jsonl." in md   # the path is not a name
    assert "**Ada:** Sage published it to Domino." in md             # the byline moved, the words did not
