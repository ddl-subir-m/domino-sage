...
def test_the_sheet_lists_the_projects_apps_and_preselects_none_of_them(tmp_path: Path):
    ...
    for row in sheet["apps"]:
        ...
        # `published`, `building`, `behind`, `resolvedMerge`, the published App's `url` and the name
...
def test_an_app_started_from_build_sits_in_the_rail_beside_the_ones_from_a_handoff(tmp_path: Path):
    ...
    # A placeholder rather than a name, because no plan has proposed one and a prompt is not a
    # name (#216) — numbered so that two rows wearing a placeholder tell somebody which is which.
    # Third of three, in birth order.
    assert {r["id"]: r["name"] for r in rows}[born] == "Draft app 3"
