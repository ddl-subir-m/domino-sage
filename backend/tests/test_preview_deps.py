...
def test_the_attach_finishes_its_remaining_work_after_a_preview_failure(tmp_path: Path, monkeypatch):
    ...
    Everything after the preview start — voicing AGENTS.md, splicing instructions, rehydrating
    attachments — has to run, or the project that gets cached is one Build cannot use.
    """
    ...
    assert (project.workspace.path / "vite.config.ts").read_text() == "// template v2\n"
