...
def remove_project_resource(id: str = "") -> JSONResponse:
    try:
        ...
        # with no app source behind it and so nothing in `refs` to stand in for it. `carriers` names
        # the served folders a Built App carries files from (ADR-0048) — the third holder, whose
        # way out is Detach rather than unbind, and which `refs` would otherwise mislabel.
        return JSONResponse(status_code=409,
                            content={"error": str(e), "apps": e.apps, "refs": e.refs,
                                     "conversations": e.conversations, "carriers": e.carriers})
    if not ok:
...
async def chat_completions(request: Request):
    ...
    def stream():
        ...
        def sniff(chunk: bytes) -> None:
            ...
            # saying the provider ended the answer early. A `SchemaError(Missing key …)` on `read`
            # or `bash` is exactly that shape, and without this line it cannot be told from
            # arguments that arrived whole and malformed — the two causes the docstring on
            # `cut_off_finish_reason` names, which no reader of the ring could separate.
            #
