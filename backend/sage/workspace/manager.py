...
class Workspace:
    ...
    def render_history_md(self) -> None:
        ...
        out = [brand.text("<!-- Generated by {assistantName} from .sage/history.jsonl. "
                          "Overwritten each turn — do not edit. -->"),
               "", "# Earlier turns", ""]
        if dropped:
    ...
    def _render_entry(entry: dict) -> list[str]:
        ...
        if kind == "agent":
            ...
            if entry.get("kind") != "text":
                return []
            return [brand.text("**{assistantName}:** {said}", said=entry.get("text", "")).rstrip(), ""]
