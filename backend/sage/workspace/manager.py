...
class Workspace:
    ...
    def render_history_md(self) -> None:
        ...
        # De-branded once, not per-pack (ADR-0014's third arm). This file is committed to the
        # user's repo as a record of a conversation that already happened; if it carried the pack's
        # words, changing packs would silently rewrite that record. Same reasoning as `build: `.
        out = ["<!-- Generated from .sage/history.jsonl. Overwritten each turn — do not edit. -->",
               "", "# Earlier turns", ""]
        if dropped:
    ...
    def _render_entry(entry: dict) -> list[str]:
        ...
        if kind == "agent":
            ...
            if entry.get("kind") != "text":
                return []
            # `Agent`, not the pack's assistantName: see the header above. CONTEXT.md puts
            # "assistant" on the avoid list, so the neutral word is `Agent`.
            return [f"**Agent:** {entry.get('text', '')}".rstrip(), ""]
