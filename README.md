...
| `SAGE_BRAND_OVERRIDE` | where Appearance (Account settings) writes the theme and the product/assistant names. **Set this on Domino**: it defaults to `~/.config/sage/brand.json`, which the container rebuilds from the image on every start, so the choice would not survive a restart. Point it at a path on a mounted Dataset — the same Dataset everywhere gives the whole deployment one answer; a per-user one follows the person between projects. See [`docs/workbench/brand.md`](docs/workbench/brand.md) |
