...
**A `CONTEXT.md` term gets a noun key if and only if it appears in a user-visible string**, and the lint computes that rather than a person maintaining a list. This is the only version of "derived from the glossary" that stays true: a new glossary term used in the UI fails the lint until it has a key, a glossary term that exists only to disambiguate — `AI Gateway`, `Domino Artifacts`, `Hosted GenAI Endpoint` — never gets one, and the list cannot drift from what the UI actually says. It also settles a question the alternative leaves open: Sage's own coinages, `Built App` and `Gallery` among them, are names a person sees, so they are replaceable too.
