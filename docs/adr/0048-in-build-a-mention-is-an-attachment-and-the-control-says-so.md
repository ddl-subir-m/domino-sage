...
`Use in this conversation` (`LeafRow`, `resource-tree.js`). What happened was that Sage copied the
file into the Built App, wrote a committed manifest entry that rehydrates it at publish, and armed
the sensitivity lock. Nothing on screen said any of it.

Four consequences followed from one click, and the fourth had no way out:

1. `add_thread_context` forked on `inBuild` and called `attach_file` (`service.py`).
...
|---|---|---|---|---|
| Folder (`resource-tree.js`) | `Attach` | `Attach this folder to {app}` | yes, `inBuild` | yes, names the app |
| File (`LeafRow`, `resource-tree.js`) | `Use here` | `Use in this conversation` | **no** | **no** |
...
**Build cannot reach scratch.** Chat's session opens on `ensure_chat_workdir` (`threads.py`), which
symlinks `examples/`, `.sage/scratch/` and the selected app's `public/data/` into one cwd. Build's
session opens on `project.app_for_turn().path` (three call sites in `service.py`). Nothing links
scratch into an app tree, so a scratch path handed to a Build turn resolves inside `apps/<id>/` and
finds nothing.

**The built app fetches over HTTP.** `_promote_chat_file` states it (`service.py`): the app a
...
(`service.py`). `.sage/attachments.json` is committed, and `_rehydrate_attached`
(`service.py`) rebuilds the links from it at the far end. Location and durability are two
...
`datasetId` and a `datasetRelPath` (`add_thread_context`, `service.py`), so exactly two controls
reach it: the Dataset **file** leaf in the rail tree (`LeafRow`, `resource-tree.js`) and the
composer `@`-menu (`composer.js`). Only the leaf has a label. In Build it reads `Attach`, with
`Attach file to {app}` as both its tooltip and its aria-label; in Chat it keeps `Use here` / `Use in
this conversation`, which is true there. The folder tooltip shortens to `Attach to {app}` so the
pair reads as one act at both grains — two phrasings for one act at two grains is the drift
...
The ink stays short. `LeafRow` already settled that (`resource-tree.js`): the glossary term is
...
**The record.** `attach_file` built its manifest record through `_dataset_entry` (`service.py`),
which wrote `dataset_id`, `dataset`, `file`, `path`, `size`, `source` and `dataset_rel_path`. No
author. (That function carries `added_by` and `conversation_id` today — #262 put them there. Read
the sentence as the shape this decision found, not the one on disk now.) The chip one layer up
carries `addedBy` (`api.js`) and the fork drops it, passing only `dataset_id` and `rel`. It now
carries `added_by` and the Conversation id across.
...
`remove_project_resource` (`service.py`) gains a third holder question, through
`_dataset_is_attached` (`service.py`) — a predicate that already exists for two other readers
...
the rule in `remove_project_resource` is that both questions are asked before either refuses: two
holders produce one refusal, and reporting one would send a person off to fix it and straight back
into a second refusal they were never warned about. A third class does not get to break that.

The sentence rolls up by Dataset folder. A folder attach writes one entry per file (ADR-0029), so
the refusal must survive a hundred of them; `FOLDER_COLLAPSE_THRESHOLD` and `_named_with_rest`
already answer that and are reused rather than reinvented.

## The lock's words

**ADR-0043 never says what scope the declared lock has.** It calls `SensitivityGate` "per Project"
and speaks of "the Project's Datasets", which reads as Project-wide and is the reading
this bug was filed under. The code is narrower: `_datasets_in_scope` (`service.py`) reads
`project.workspace.read_bindings()` and `project.attached`, and `Project.attached` is the app on
screen (`service.py`). **It is the selected app's**, and that is stated here because an
...
`_ensure_dir_link` re-points when the selection changes (`threads.py`). Widening it to every app
...
(`service.py`); `LockNotice` reads it only for a plural count (`composer.js`) and prints
the brand word instead. `declaredPhrase` (`util.js`) already builds the named sentence and
...
file.** Rejected on `_data_usage` (`service.py`), whose own docstring says `refs` "errs
...
- **Two controls that look like doors here are not, and are deliberately untouched.** The table leaf
  one branch over (`resource-tree.js`) is `kind: "table"` and never reaches the fork. The details
