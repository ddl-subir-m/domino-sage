...
Of those, only the two that are deliberate acts in their own right — New conversation, and deleting one — still commit inside the request. Opening a *different* Thread hands its predecessor's commit to the timer instead of running it first, because that read is the one a person is waiting on and the commit is two network round trips they did not ask for (#456). The commit still happens without another request arriving: the timer is a thread, and a graceful stop flushes whatever is left. What a queued save loses is a caller to answer, so the last one that failed is kept and handed back on the next `GET /api/threads/<id>` as `saveFailed` (`null` when the last save landed) — retrying on the idle timer is not the same as telling anyone, and an unnoticed failure to commit someone's work is worse than a slow click.
...
- The chip is drawn before the server answers, muted (`pending`) until the row is on the Thread; a second pick of the same Resource during that wait is not a second row, on either side (#486). A table chip's column read runs beside the attach, not on it, and the next turn waits for it (bounded, `_CHIP_COLUMNS_WAIT_S`) before rendering the prompt. Sending waits for chips still landing.
